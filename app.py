...
def update_cart():
    ...
    # Graceful parsing; cart handles <=0 by removing
    try:
        quantity = int(request.form.get('quantity', 1))
    except (TypeError, ValueError):
        quantity = 1
...
def process_checkout():
    ...
    discount_key = discount_code.upper()
    
    # Calculate total with discount
    total_amount = cart.get_total_price()
    discount_applied = 0
    
    if discount_key == 'SAVE10':
...
def register():
    ...
    if request.method == 'POST':
        ...
        # Basic email format validation (simple regex for demo)
        email_regex = r"^[^@\s]+@[^@\s]+\.[^@\s]+$"
        ...
        # Create new user
        user = User(email_key, password, name, address)
        users[email_key] = user
        
        # Log in the user
        session['user_email'] = email_key
